...
oracle='oracle_heavyTailedStar';
xstart=[-1;-1];

%set up LpAdaptation options
...
inopts.para_hitP_adapt.PVec = [0.35,0.20,0.07];
...
inopts.para_hitP_adapt.maxEvalSchedule = (1/3)*ones(1,3);%[1/2,1/8,1/8,1/8,1/8];%[3/5,3/50,4/50,6/50,7/50]; % proportion of maxEval 
...
inopts.para_hitP_adapt.numLastSchedule = [0.5,0.5,0.5];%[1/2,3/4,3/4,3/4,3/4]; % over how many 
...
[a,b]=size(X1);
sample = [X1(:),Y1(:)];
inside1 = oracle_heavyTailedStar(sample);
